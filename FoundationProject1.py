...
def save_to_db(person, animaldb):
    ...
    animaldb.users.insert_one(dict_User)    # Insert into database, animal collection in animal database
...
def set1():        # Drop database function
    ...
    animaldb = client.pyproject

    movies = animaldb.users  
...
def main():
    ...
    #movies = animaldb.animals  # Define a collection

    #movie = movies.update_one( {"name" : "m" }, {"Set" :{"funds" : "7000"}})
